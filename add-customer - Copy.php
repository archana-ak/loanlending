...
if($count>0){
	...
	{
$query=mysqli_query($mysqli,"insert into customer(addedby,name,address,personal_no,office_no,email,email_personal,business_name,state,district,city,pin,profession,added_id) values('$add','$full_name','$address','$personal_no','$office_no','$mail_ID','$mail_ID_personal','$business_name','$state','$district','$city','$pin','$profession','$addid')");
...
<body>
  ...
  $stmt= $mysqli->prepare($ret) ;
   ...
   $res=$stmt->get_result();
    ...
    {
      ?>  
	<input type="hidden" name="addedby"  value="<?php echo $r->name;?>">
		<input type="hidden" name="addedid"  value="<?php echo $r->id;?>"><?php } ?>
